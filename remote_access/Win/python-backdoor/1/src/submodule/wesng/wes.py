...
def main(args, tmp_dir):
    ...
    if args['perform_update']:
        ...
        urlretrieve('https://raw.githubusercontent.com/bitsadmin/wesng/master/definitions.zip',
                    f'{tmp_dir}/definitions.zip')
        cves, date = load_definitions(f'{tmp_dir}/definitions.zip')
    ...
    # Using systeminfo.txt or qfe.txt with list of installed patches as a base
        ...
        if args['qfefile']:
            ...
            if (args['operating_system'] and args['operating_system'].isdigit()) or \
                    (not args['operating_system']):
    ...
    if len(filtered) > 0:
        ...
        print(colored('[+] Done. ', 'green') + '%s %s of the %s vulnerabilities found.' % (
        verb, colored(len(filtered), 'yellow'), colored(len(found), 'yellow')))
    else:
...
def apply_display_filters(found, hiddenvulns, only_exploits, impacts, severities):
    ...
    for cve in found:
        ...
        for hidden in hiddenvulns:
            if hidden in cve['AffectedComponent'].lower() or hidden in cve['AffectedProduct'].lower() or hidden in cve[
                'Title'].lower():
                add = False
                break
...
def determine_product(systeminfo):
    ...
    # OS Version
    regex_version = re.compile(r'.*?((\d+\.?){3}) ((Service Pack (\d)|N\/\w|.+) )?[ -\xa5]+ (\d+).*',
    ...
    # OS Name
    win_matches = re.findall('.*?Microsoft[\(R\)]{0,3} Windows[\(R\)?]{0,3} ?(Serverr? )?(\d+\.?\d?( R2)?|XP|VistaT).*',
                             systeminfo, re.MULTILINE | re.IGNORECASE)
...
def print_summary(kbs, sp):
    ...
    if len(missingpatches) > len(grouped):
        ...
        print(colored(
            '[I] Check the details of the unknown patches at https://support.microsoft.com/help/KBID,\n    for example https://support.microsoft.com/help/890830 in case of KB890830',
            'yellow'))
...
def print_results(results):
    ...
    for res in results:
        ...
        print('Date: %s\n'
              ...
              'Severity: %s\n'
              'Impact: %s\n'
              '%s: %s\n' % (res['DatePosted'], res['CVE'], res['BulletinKB'], res['Title'], res['AffectedProduct'],
                            res['AffectedComponent'], colored(res['Severity'], highlight), res['Impact'], label, value))
...
def check_definitions_exists(value):
    if not os.path.isfile(value):
        raise argparse.ArgumentTypeError(
            'Definitions file \'%s\' does not exist. Try running %s --update first.' % (value, FILENAME))
